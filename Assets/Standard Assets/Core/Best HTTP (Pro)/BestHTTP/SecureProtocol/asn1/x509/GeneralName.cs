...
namespace Standard_Assets.Core.BestHTTP.SecureProtocol.asn1.x509
{
        ...
        : Asn1Encodable, IAsn1Choice
    {
		...
		private byte[] toGeneralNameEncoding(
			string ip)
		{
			if (SecureProtocol.util.net.IPAddress.IsValidIPv6WithNetmask(ip) || SecureProtocol.util.net.IPAddress.IsValidIPv6(ip))
			{
			...
			else if (SecureProtocol.util.net.IPAddress.IsValidIPv4WithNetmask(ip) || SecureProtocol.util.net.IPAddress.IsValidIPv4(ip))
			{
				int slashIndex = ip.IndexOf('/');
