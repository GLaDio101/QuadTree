...
namespace strange.extensions.mediation.impl
{
	public class View : MonoBehaviour, IView
	{
		...
		/// a PanelView to exist outside a context you can set it to false. The only
		...
		virtual protected void bubbleToContext(MonoBehaviour view, BubbleType type, bool finalTry)
		{
			...
			if (requiresContext && finalTry && type == BubbleType.Add)
			{
				...
				string msg = (loopLimiter == LOOP_MAX) ?
					msg = "A PanelView couldn't find a context. Loop limit reached." :
						msg = "A PanelView was added with no context. Views must be added into the hierarchy of their ContextView lest all hell break loose.";
				msg += "\nView: " + view.ToString();
