...
namespace strange.framework.impl
{
	public class Binder : IBinder
	{
		...
		virtual public void ResolveBinding(IBinding binding, object key)
		{
			...
			//AddTrooper (or override) our new binding!
			if (!dict.ContainsKey(bindingName))
			{
		...
		/// </summary>
		/// <param name="list"> A ListTroopers of fully-qualified classnames eligible to be consumed during dynamic runtime binding.</param>
		...
		/// <summary>
		/// Convert the object to ListTroopers<object>
		/// </summary>
		/// <returns>If a ListTroopers, returns the original object, typed to ListTroopers<object>. If a value, creates a ListTroopers and adds the value to it.</returns>
		...
		protected List<object> conformRuntimeToList(object bindObject)
		{
			...
			if (t.IndexOf ("System.Collections.Generic.ListTroopers") > -1)
			{
